...
class API_Runs(APIView):
    ...
    def get(
        ...
        if id == "game":
            response_data["game_runs"] = self._get_game_runs(embed_fields)
        else:
...
class API_RunsGame(APIView):
    """List runs for a specific game slug with optional filtering.

    Query params:
      - type: 'full-game' or 'il'
      - category: matches Runs.subcategory (FG) OR Levels.name (IL)
      - player: player id or exact name
      - limit: 1-200 (default 50)
      - ordering: time, -time, date, -date, points, -points
      - embed: comma list of allowed embeds
    """
    permission_classes = [ReadOnlyOrAuthenticated]

    def get(self, request: HttpRequest, game_slug: str) -> HttpResponse:
        game = Games.objects.filter(slug__iexact=game_slug).first()
        if not game:
            return Response({"ERROR": "Game slug not found."}, status=status.HTTP_404_NOT_FOUND)

        qs = Runs.objects.filter(game=game).select_related("player", "player__countrycode", "game")

        # Exclude rejected/new and obsolete runs by default (only show current PBs)
        include_obsolete = request.GET.get("include_obsolete") == "1"
        if not include_obsolete:
            qs = qs.exclude(vid_status__in=["new", "rejected"]).filter(obsolete=False)

        run_type = request.GET.get("type")
        if run_type == "full-game":
            qs = qs.filter(level__isnull=True)
        elif run_type == "il":
            qs = qs.filter(level__isnull=False)

        category = request.GET.get("category")
        if category:
            qs = qs.filter(
                Q(subcategory__icontains=category)
            )

        player_param = request.GET.get("player")
        if player_param:
            player_obj = Players.objects.filter(Q(id__iexact=player_param) | Q(name__iexact=player_param)).first()
            if player_obj:
                qs = qs.filter(player=player_obj)
            else:
                return Response({"ERROR": "Player not found."}, status=status.HTTP_404_NOT_FOUND)

        # Optional platform filter (accepts platform id or case-insensitive name)
        platform_param = request.GET.get("platform")
        if platform_param:
            qs = qs.filter(Q(platform__id__iexact=platform_param) | Q(platform__name__iexact=platform_param))
        ...
        # Determine timing field used for evaluating "best" runs
        timing_pref = game.idefaulttime if run_type == "il" else game.defaulttime
        timing_field_map = {
            "realtime": "time_secs",
            "realtime_noloads": "timenl_secs",
            "ingame": "timeigt_secs",
        }
        timing_field = timing_field_map.get(timing_pref, "time_secs")

        best_only = request.GET.get("best_only", "1") == "1"

        # If we only want best per player, pre-order by timing then date for deterministic selection.
        if best_only:
            # Order earlier for stable selection of fastest then earliest date tie-breaker
            qs = qs.order_by(timing_field, "date")
            seen_players = set()
            best_runs = []
            for run in qs:
                pid = run.player_id
                if not pid:  # skip runs without a player
                    continue
                # Only consider runs with a valid timing metric (non-null & >0) else take first encountered
                if pid not in seen_players:
                    best_runs.append(run)
                    seen_players.add(pid)
            # Apply requested ordering AFTER selecting best runs
            order_field = ordering_map.get(ordering, "-date")
            reverse = order_field.startswith("-")
            key_field = order_field.lstrip("-")
            if order_field in ("time_secs", "-time_secs", "timenl_secs", "-timenl_secs", "timeigt_secs", "-timeigt_secs"):
                # Already sorted by timing ascending; handle reverse case
                if reverse:
                    best_runs.sort(key=lambda r: getattr(r, timing_field) or float("inf"), reverse=True)
                else:
                    best_runs.sort(key=lambda r: getattr(r, timing_field) or float("inf"))
            else:
                best_runs.sort(key=lambda r: getattr(r, key_field) or 0, reverse=reverse)
            ...
            qs = qs.order_by(ordering_map.get(ordering, "-date"))
            try:
                limit = int(request.GET.get("limit", 50))
            except ValueError:
                limit = 50
        ...
        embed_fields = [f.strip() for f in request.GET.get("embed", "").split(",") if f.strip()]
        invalid_embeds = [f for f in embed_fields if f not in API_Runs.ALLOWED_EMBEDS]
        if invalid_embeds:
            return Response({"ERROR": f"Invalid embed(s): {', '.join(invalid_embeds)}"}, status=status.HTTP_400_BAD_REQUEST)

        data = RunSerializer(qs, many=True, context={"embed": embed_fields}).data
        return Response({
...
class API_GameSubcategories(APIView):
    """Return distinct subcategories for a game (optionally filtered by category type).

    Query params:
      - type: per-level | per-game (optional)
      - include_counts: '1' to include run counts per subcategory
    """
    permission_classes = [ReadOnlyOrAuthenticated]
    ...
    def get(self, request: HttpRequest, game_slug: str) -> HttpResponse:
        ...
        if cat_type and cat_type not in {"per-level", "per-game"}:
            return Response({"ERROR": "Invalid type. Use per-level or per-game."}, status=status.HTTP_400_BAD_REQUEST)

        qs = Runs.objects.filter(game=game, subcategory__isnull=False).exclude(subcategory="")
        if cat_type:
            qs = qs.filter(category__type=cat_type, category__isnull=False)
        else:
            # Exclude runs without a category only when counting by type not requested? Keep all for completeness.
            qs = qs.filter(category__isnull=False)

        include_counts = request.GET.get("include_counts") == "1"

        if include_counts:
            sub_qs = (
                qs.values("subcategory")
                .annotate(run_count=Count("id"))
                .order_by("subcategory")
            )
            subcategories = [
                {"name": row["subcategory"], "run_count": row["run_count"]}
                for row in sub_qs
            ]
        else:
            subcategories = list(
                qs.values_list("subcategory", flat=True).distinct().order_by("subcategory")
            )
