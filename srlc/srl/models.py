...
def validate_image(image):
    ...
    if file_size > limit_mb * 1024 * 1024:
        ...
        raise ValidationError(f"File width/height must match. Current: {file_width}x{file_height}")
...
class Games(models.Model):
    ...
    twitch          = models.CharField(max_length=55, verbose_name="Twitch Name", null=True,
                                        blank=True)
    release         = models.DateField(verbose_name="Release Date")
    boxart          = models.URLField(verbose_name="Box Art URL")
    defaulttime     = models.CharField(verbose_name="Default Time", choices=LeaderboardChoices,
                                        default="realtime")
    idefaulttime    = models.CharField(
                    verbose_name="ILs Default Time",
                    choices=LeaderboardChoices,
                    default="realtime",
                    help_text=("Sometimes leaderboards have one timing standard for full game "
    ...
    pointsmax       = models.IntegerField(
                    ...
                    default=1000,
                    help_text=("Default is 1000; 25 if this game contains the name \"Category "
    ...
    ipointsmax      = models.IntegerField(
                    ...
                    default=100,
                    help_text=("Default is 100; 25 if this game contains the name \"Category "
...
class Categories(models.Model):
    ...
    game    = models.ForeignKey(Games, verbose_name="Linked Game", null=True,
...
class Variables(models.Model):
    ...
    game        = models.ForeignKey(Games, verbose_name="Linked Game", null=True,
                                    on_delete=models.SET_NULL)
    cat         = models.ForeignKey(Categories, verbose_name="Category", null=True,
                                    blank=True, on_delete=models.SET_NULL)
    all_cats    = models.BooleanField(
                verbose_name="All Categories",
                default=False,
                help_text=("When checked,  this means that the variable will work across all "
    ...
    def clean(self):
        if (self.cat is None) and (not self.all_cats):
            raise ValidationError("\"Linked Category\" must have a value OR \"All Categories\" must\
                                be checked.")
        elif self.cat == "all":
            raise ValidationError("\"all\" is not a valid category value; if this variable is for\
                                all categories,  please check \"All Categories\".")
        elif self.cat and self.all_cats:
            raise ValidationError("If \"All Categories\" is checked,  \"Linked Category\" field\
                                should be empty.")
...
class VariableValues(models.Model):
    ...
    var     = models.ForeignKey(Variables, verbose_name="Linked Variable", null=True,
                                on_delete=models.SET_NULL)
    name    = models.CharField(max_length=50, verbose_name="Name")
...
class Awards(models.Model):
    ...
    image       = ResizedImageField(
                ...
                null=True,
                blank=True,
                help_text=("Note: Images must be at least 64px in size, must be a square (height "
                "and width must match), and the max filesize is 3MB.")
    )
    description = models.CharField(max_length=500, verbose_name="Award Description", blank=True,
                                    null=True)
    unique      = models.BooleanField(
                verbose_name="Unique Award",
                default=False,
                help_text=("When checked, this award will be given the 'unique' tag - enabling "
...
class Players(models.Model):
    ...
    nickname    = models.CharField(
                ...
                blank=True,
                null=True,
                help_text=("This is a special field where,  if a nickname is given, it will be "
                "shown versus their SRC name.")
    )
    url         = models.URLField(verbose_name="URL")
    countrycode = models.ForeignKey(CountryCodes, verbose_name="Country Code", blank=True,
                                    null=True, on_delete=models.SET_NULL)
    pfp         = models.CharField(max_length=50, verbose_name="Profile Picture URL", blank=True,
                                    null=True)
    ...
    ex_stream   = models.BooleanField(
                ...
                default=False,
                help_text=("When checked, this player can be filtered out from appearing on stream "
                "bots or pages.")
    )
    awards      = models.ManyToManyField(
                ...
                default=False,
                blank=True,
                help_text=("Earned awards can be selected here. All selected awards will appear on "
...
class Runs(models.Model):
    ...
    runtype         = models.CharField(max_length=5, choices=runtype,
    ...
    category        = models.ForeignKey(Categories, verbose_name="Category", blank=True, null=True,
                                        on_delete=models.SET_NULL)
    level           = models.ForeignKey(Levels, verbose_name="Level", blank=True, null=True,
                                        on_delete=models.SET_NULL)
    subcategory     = models.CharField(max_length=100, verbose_name="Subcategory Name", blank=True,
                                       null=True)
    variables       = models.ManyToManyField(Variables, verbose_name="Variables",
                                             through="RunVariableValues", related_name="runs")
    player          = models.ForeignKey(Players, verbose_name="Player", blank=True, null=True,
                                        on_delete=models.SET_NULL, related_name="runs_p1")
    player2         = models.ForeignKey(Players, verbose_name="Player 2", blank=True, null=True,
                                        on_delete=models.SET_NULL, related_name="runs_p2")
    ...
    time            = models.CharField(max_length=25, verbose_name="RTA Time", blank=True,
                                        null=True)
    time_secs       = models.FloatField(verbose_name="RTA Time (Seconds)", blank=True, null=True)
    timenl          = models.CharField(max_length=25, verbose_name="LRT Time", blank=True,
                                        null=True)
    timenl_secs     = models.FloatField(verbose_name="LRT Time (Seconds)", blank=True, null=True)
    timeigt         = models.CharField(max_length=25, verbose_name="IGT Time", blank=True,
                                        null=True)
    ...
    platform        = models.ForeignKey(Platforms, verbose_name="Platform", blank=True, null=True,
                                        on_delete=models.SET_NULL)
    ...
    vid_status      = models.CharField(
                    ...
                    default="verified",
                    help_text=("This is the current status of the run, according to Speedrun.com. "
                    "It should be updated whenever the run is approved. Runs set as \"Unverified\" "
                    "or \"Rejected\" do not appear anywhere on this site.")
    )
    approver        = models.ForeignKey(Players, verbose_name="Approver", blank=True, null=True,
                                        on_delete=models.SET_NULL, related_name="approver")
    obsolete        = models.BooleanField(
                    verbose_name="Obsolete?",
                    default=False,
                    help_text=("When True, the run will be considered obsolete. Points will not "
    ...
    arch_video      = models.URLField(
                    ...
                    blank=True,
                    null=True,
                    help_text=("Optional field. If you have a mirrored link to a video, you can "
                    "input it here.")
    )
...
class NowStreaming(models.Model):
    ...
    streamer    = models.OneToOneField(Players, primary_key=True, verbose_name="Streamer",
                                        on_delete=models.CASCADE)
    game        = models.ForeignKey(Games, verbose_name="Game", null=True,
                                    on_delete=models.SET_NULL)
    title       = models.CharField(max_length=100, verbose_name="Twitch Title")
    offline_ct  = models.IntegerField(
                verbose_name="Offline Count",
                help_text=("In some situations, bots or the Twitch API can mess up. To help users, "
